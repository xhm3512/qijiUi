...
var RemarkText = function RemarkText(props) {
          ...
          if (key === 'line') {
            ...
            tempText = tempText.replace(
              ...
              '<font color="#ffffff" style="background-color: '
                .concat(_constants.colorHight[key], ';">')
                .concat(item, '</font>'),
            );
          }
        });
          ...
          words.forEach(function(item) {
            ...
            if (key === 'line') {
              ...
              tempText = tempText.replace(
                reg,
                '<font color='
                  .concat(_constants.colorHight16[key], '>')
                  .concat(item, '</font>'),
              );
            }
          });
  ...
  var getWords = function getWords() {
    ...
    filterDom.forEach(function(item) {
      ...
      var backColor =
        styleBack && (0, _tools.getStyle)(item, 'background-color');
      var tempLength = tempWordsArr.length;
      var colorType = _constants.colorRgbHight[backColor];
      var flag = true;
