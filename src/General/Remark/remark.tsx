...
const RemarkText = (props: PropsRoot) => {
      ...
      Object.keys(machineWords).map(key => {
        ...
        if (words && words.length > 0) {
          words.forEach((item: string | RegExp) => {
            ...
            const reg = new RegExp(item, 'g');
            if (key === 'line') {
              tempText = tempText.replace(reg, `<strike  data-check='1'>${item}</strike>`);
            } else {
              tempText = tempText.replace(reg, `<font color=${colorHight16[key]}>${item}</font>`);
            }
          });
        }
      });
    ...
    filterDom.forEach((item: HTMLElement) => {
      ...
      tempWordsArr.forEach((tempWords, i) => {
        if (
          ...
          ((tempWords.type === nodeName && nodeName === 'STRIKE') || tempWords.type === colorType)
        ) {
          // 表示是相邻两个统一类型的标志
          tempWordsArr[i] = {
  ...
  const onHightChange = (oldDomHtml: any) => {
    ...
    if (typeof onSubmitHtml === 'function') { onSubmitHtml({ newDomHtml, highwordsArr, machineWords }); }
  ...
  useEffect(() => {
    ...
    if (typeof onSubmitHtml === 'function') { onSubmitHtml({ newDomHtml: text, highwordsArr: manSpamWords, machineWords }); }
  }, []);
